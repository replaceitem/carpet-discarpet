...
public class ValueFromId {
	...
	public Message dc_message_from_id(Context ctx, String messageId, Channel channel) {
		...
		return ValueUtil.awaitFuture(bot.getJda().getMessageById(messageId, textChannel), "Error getting message from id");
	}
	...
	public Webhook dc_webhook_from_id(Context ctx, String webhookId, String token) {
		...
		return ValueUtil.awaitFuture(bot.getJda().getIncomingWebhookByIdAndToken(webhookId, token), "Error getting webhook from id");
	}
	...
	public Webhook dc_webhook_from_url(Context ctx, String webhookUrl) {
		...
		return ValueUtil.awaitFuture(bot.getJda().getIncomingWebhookByUrl(webhookUrl), "Error getting webhook from url");
	}
}
