...
public class ServerValue extends DiscordValue<Guild> implements Renamable {
    ...
    public Value getProperty(String property) {
        return switch (property) {
            ...
            case "slash_commands" -> ListValue.wrap(ValueUtil.awaitRest(delegate.retrieveCommands(), "Error getting slash commands from server").stream().map(SlashCommandValue::of));
    ...
    public void rename(String name) {
        ValueUtil.awaitRest(delegate.getManager().setName(name), "Could not rename server");
    }
}
