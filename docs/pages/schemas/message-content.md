...
|      `use_components_v2` {:?} | Boolean                                                                | Whether to use the components v2 system. Defaults to `false`. This enabled usage of the newer components. When set, fields like `content` and `embeds` are not allowed. All content must be sent through components. |
